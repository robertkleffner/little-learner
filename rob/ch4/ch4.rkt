...
(gradient-of (λ (theta) (sqr (ref theta 0))) (list 27.0))

(define obj-fun ((l2-loss line) line-xs line-ys))

(gradient-of obj-fun (list 0.0 0.0))
...
(revise (λ (theta) (map (λ (p) (- p 3)) theta)) 5 (list 1 2 3))
...
;; using map instead of hardcoded for two parameters
(let ([alpha 0.01]
      [obj obj-fun])
  (let ([f (λ (theta)
             (let ([gs (gradient-of obj theta)])
